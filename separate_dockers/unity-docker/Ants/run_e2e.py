...
for x,map_path in enumerate(MAPS_PATHS):
    ...
    its=ITERATIONS[x]
    for y in range(reps):
        subprocess.run(["./Ants_LinuxServerBuild.x86_64", map_path,"--iterationCount="+its, "--repetitionsCount=1"])
    ...
    print("Finished Bootups!, map: <<{}>>, rep: <<{}>>, time: <<{}>>, fps<<{}>>".format(map_path, reps, total_time, fps), flush=True)
